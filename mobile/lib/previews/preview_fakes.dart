...
class FakeNotificationRepository implements NotificationRepository {
  ...
  })  : _items = items ?? List.from(PreviewData.notifications),
        _prefs = prefs ?? PreviewData.notificationPreferences;
  ...
  Future<List<NotificationItemEntity>> getNotifications({
    ...
    if (isRead != null) {
      return _items.where((n) => n.isRead == isRead).toList();
    }
    return _items;
  }
  ...
  Future<NotificationItemEntity> markAsRead(String notificationId) async {
    ...
    if (idx != -1) {
      final updated = _items[idx].copyWith(isRead: true, readAt: DateTime.now());
      _items[idx] = updated;
      return updated;
    }
    return _items.first;
  ...
  Future<int> markAllAsRead() async {
    final count = _items.where((n) => !n.isRead).length;
  ...
  @override
  Future<NotificationPreferencesEntity> getPreferences() async => _prefs;
...
class PreviewNotificationInboxNotifier extends NotificationInboxNotifier {
  PreviewNotificationInboxNotifier([NotificationInboxState? initialState])
      : super(FakeNotificationRepository()) {
    state = initialState ??
...
class PreviewNotificationPreferencesNotifier
    extends NotificationPreferencesNotifier {
  PreviewNotificationPreferencesNotifier([
    NotificationPreferencesState? initialState,
  ]) : super(FakeNotificationRepository()) {
    state = initialState ??
        const NotificationPreferencesState(
          status: NotificationStatus.loaded,
          preferences: PreviewData.notificationPreferences,
        );
  }
